...
export default function DarkVeil({
  ...
  const reference = useRef<HTMLCanvasElement | null>(
    undefined as unknown as HTMLCanvasElement | null,
  );
  useEffect(() => {
    let renderer: OglRenderer | undefined;
    let mesh: OglMesh | undefined;
    ...
    const start = async () => {
      ...
      const loop = () => {
        uniforms["uTime"].value =
          ((globalThis.performance.now() - startTime) / 1000) * speed;
  ...
  return <canvas ref={reference} className="w-full h-full block" />;
}
